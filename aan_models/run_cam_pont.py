...
if __name__ == '__main__':
    ...
    max_pot = [5]
    optimize_sim = False
    # no_rand_aircrafts=[1, 0, 1, 0] !! COUPLED TO LINE 122
    # no_aircrafts_from_sides=[[1,0,0,1],[1,1,0,0],[1,0,0,1],[0,1,1,1]] # [UP, DOWN, LEFT, RIGHT] !! COUPLED TO LINE 123
    # max_size=[1,1,1,1]
    # max_pot=[1,1,1,1]
    ...
    for nra,nas,ms,mp in zip(no_rand_aircrafts,no_aircrafts_from_sides,max_size,max_pot):
        ...
        for show_single_clip in show_vis_clip:
            ...
            tma = TMA(row=10,column=10,max_pot=60,grid_size=grid_size)
            ...
            w3 = Waypoint(row=15, column=5, max_pot=7, grid_size=grid_size, size=2, inf_rad=3)
            w4 = Waypoint(row=15, column=15, max_pot=7, grid_size=grid_size, size=2, inf_rad=3)

            sob1 = Stat_Obstruction(row=2, column=10, max_pot=5, grid_size=grid_size, size=2, inf_rad=4)

            mob1 = Mov_Obstruction(row=7, column=8, max_pot=15, grid_size=grid_size, size=1, inf_rad=3, 
                                   mob_rate=5, mob_span=2, mut_rate=5, mut_span=1)
            mob2 = Mov_Obstruction(row=13, column=8, max_pot=15, grid_size=grid_size, size=1, inf_rad=3, 
                                   mob_rate=5, mob_span=2, mut_rate=5, mut_span=1)
